...
fn parameter(identifier: &str, name: &str) -> Value {
    ...
    match name {
        ...
        "logfmt_source_override" => {
            Value::Text("host=web-1 status=200 empty= status=500".to_owned())
        }
        "logfmt_source_path" => Value::Text("$.logfmt".to_owned()),
        "unpack_logfmt_name_1" => Value::Text("host".to_owned()),
        ...
        "running_group_path" => Value::Text("$.service".to_owned()),
        ...
        "histogram_path" => Value::Text("$.duration_ms".to_owned()),
...
fn semantic_regressions(connection: &Connection, recipes: &[Recipe]) -> Result<()> {
    ...
    if unpack_logfmt_rows
        != [
            vec![
                Value::Integer(1000),
                Value::Text("web-1".to_owned()),
                Value::Text("500".to_owned()),
                Value::Text(String::new()),
                Value::Text(String::new()),
            ],
            vec![
    ...
    let running_state = running_stats_rows
        .iter()
        .map(|row| {
            (
                row.first().cloned(),
    ...
    if histogram_rows
        != [vec![Value::Text(
            r#"[{"vmrange":"3.594e+00...4.084e+00","hits":1},{"vmrange":"1.136e+01...1.292e+01","hits":1}]"#
                .to_owned(),
        )]]
    {
