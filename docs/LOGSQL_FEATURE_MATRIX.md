...
| `LQL-F22` | `contains_any` | in progress | no | `ROWS` | `API` | P2 | Pin case-sensitive any-phrase semantics, typed projection, Unicode word boundaries, empty-list false versus empty-value true, static-list grammar, and strict subquery/error boundaries before reusing the measured decode-first API plan. No extension primitive until measured. |
