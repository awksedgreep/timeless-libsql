...
| `LQL-F35` | stream selector `{...}` | deferred | no | none | `DEFER` | DEFER | Four pinned cases prove that VictoriaLogs selects ingestion-declared, tenant-scoped stream identities—not ordinary row fields. The POST LogsQL route rejects selectors at their source position before storage across base, filter-pipe, shutdown, and reopen paths; quoted/commented braces and explicit `rows({...})` inline data remain valid. The native-parameter GET route does not accept LogsQL text. Shipping requires a versioned stream-field/tenant/canonical-ID/index/migration contract spanning every public batch and readable codec; see `QSF-007`, `QSF-261`, and `QSF-262`. |
| `LQL-F36` | `_stream_id` filter | deferred | no | none | `DEFER` | DEFER | Six accepted and two error witnesses pin VictoriaLogs' reserved, tenant-scoped 48-hex block identity: exact/quoted/static/query-backed forms, case-insensitive unquoted spelling, whitespace, and strict malformed IDs. Timeless rejects top-level `_stream_id:` filters at their source position before storage across base, filter-pipe, shutdown, and reopen paths; bare/message values, nested `foo._stream_id`, projections, comments, and explicit `rows({...})` data remain valid. Shipping requires the versioned stream identity and index contract shared with `LQL-F35`; see `QSF-007` and `QSF-263`. |
...
| `LQL-P03` | timestamp `sort` / `order by (_time)` asc/desc ([SQL](QUERY_SQL_EQUIVALENTS.md#sql-log-001-bounded-filter-sort-and-pagination)); other sort keys, including `sort by (service)` after grouped stats, are unsupported | shipped | yes | `ROWS`, `SQL` | `API` | P0 |
...
| `LQL-P09` | `stats` incl. `stats by (fields...)` grouped aggregation ([count SQL](QUERY_SQL_EQUIVALENTS.md#sql-log-003-exact-count), [bucket SQL](QUERY_SQL_EQUIVALENTS.md#sql-log-006-counts-by-field-and-time-bucket)); scalar counts use native count execution regardless of output alias, including supported following row transforms; preceding cardinality-changing transforms and request-local diagnostic reports retain the row pipeline | shipped | partial | `COUNT`, `SQL` | `API` | P0 |
