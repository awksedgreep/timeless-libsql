...
| `LQL-F17` | multi-exact filter | in progress | no | `ROWS` | `API` | P2 | Pin static `in(v1, ..., vN)` exact textual membership first. `in()` is empty, `in(*)` is a no-op, and subquery membership remains independently owned by `LQL-F38`. Plan indexed values as a posting-list union only where public contracts and measurements justify it. |
