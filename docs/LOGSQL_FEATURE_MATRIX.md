...
| `LQL-F30` | `eq_field`, `le_field`, `lt_field` ([SQL](QUERY_SQL_EQUIVALENTS.md#sql-log-021-same-row-textual-field-comparison)) | in progress | no | `ROWS`, `SQL` | `API` | P2 | Twelve successful and twelve error pinned cases plus failing-then-passing parser and real-extension regressions establish same-row exact textual equality and math-value-or-bytewise ordering. Missing/null/empty projection, retained rich values and exact large integers, quoted/message/service/nested/right-`_time` fields, aliases, logical/pipeline composition, strict errors, limits, cancellation, durability, and reopen pass. Executable `SQL-LOG-021` supplies complete retained-model equality and the bytewise ordering fallback; exact-build narrow/wide evidence remains before shipment. |
