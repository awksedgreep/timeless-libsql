...
| `LQL-F16` | exact-prefix filter ([SQL](QUERY_SQL_EQUIVALENTS.md#sql-log-014-exact-prefix)) | in progress | partial | `ROWS`, `SQL` | `API` | P2 | Decode-first candidate; pin both `="prefix"*` and the equivalent `exact(prefix*)` form. Ordinary SQL exactly covers message and retained-text prefixes; the API owns VictoriaLogs textual projection for all rich types. |
