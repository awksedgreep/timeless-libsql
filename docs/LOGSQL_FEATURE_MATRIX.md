...
| `LQL-F20` | field no-op filter ([SQL](QUERY_SQL_EQUIVALENTS.md#sql-log-016-field-no-op)) | shipped | no | `ROWS`, `SQL` | `API` | P2 | Any standalone unquoted wildcard in case-insensitive `in`, `contains_any`, or `contains_all` is a field-independent true predicate, including mixed lists, missing fields, `service`/`level` aliases, logical composition, and pipelines. Eleven pinned cases and the real extension pass; non-wildcard contains values and subqueries remain explicit `LQL-F21`/`LQL-F22`/`LQL-F38` errors. Ordinary SQL omits the field predicate, and exact-build evidence proves no new extension primitive is warranted. |
| `LQL-F21` | `contains_all` | in progress | no | `ROWS` | `API` | P2 | Fifteen successful and two error pinned cases cover case-sensitive all-phrase matching, Unicode word boundaries, static-list grammar, field scope, empty-list/empty-value identity, aliases, logical/pipeline composition, and strict errors. Real-extension regressions preserve rich typed/nested projection, work limits, cancellation, durability, and reopen; query-backed lists remain `LQL-F38`. Portable SQLite has no exact Unicode-category phrase-boundary predicate, so this row intentionally makes no SQL claim; exact-build measurement remains before shipment. |
