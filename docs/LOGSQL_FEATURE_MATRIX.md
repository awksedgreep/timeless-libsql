...
| `LQL-F27` | string-range filter ([SQL](QUERY_SQL_EQUIVALENTS.md#sql-log-019-bytewise-string-range-over-retained-text)) | in progress | no | `ROWS`, `SQL` | `API` | P2 | Sixteen successful and seven error pinned cases now cover lower-inclusive/upper-exclusive byte ordering, equal-prefix/inverted ranges, ASCII/UTF-8, typed projection, missing/null/empty, nested/message/service fields, aliases, composition, and strict errors. Real-extension and executable string-only SQL regressions pass; exact-build evidence remains before shipment. |
