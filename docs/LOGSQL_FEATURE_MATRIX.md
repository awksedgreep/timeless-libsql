...
| `LQL-F20` | field no-op filter ([SQL](QUERY_SQL_EQUIVALENTS.md#sql-log-016-field-no-op)) | in progress | no | `ROWS`, `SQL` | `API` | P2 | Pin the field-independent `field:in(*)`, `field:contains_any(*)`, and `field:contains_all(*)` true predicates without prematurely shipping the non-wildcard `LQL-F21`/`LQL-F22` value semantics or `LQL-F38` subqueries. Ordinary SQL omits the field predicate. |
