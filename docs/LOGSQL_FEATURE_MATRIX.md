...
| `LQL-F38` | subquery/`in(...)` filter ([SQL](QUERY_SQL_EQUIVALENTS.md#sql-log-048-query-backed-exact-membership)) | in progress | no | `ROWS`, `SQL` | `API` | P3 | Correctness complete; exact-build evidence is pending. Fourteen successful and five error pinned cases plus parser and 8,192-entry real-extension regressions cover exact/`uniq` output, rich and missing projection, empty identities, `in`/`contains_any`/`contains_all`, nesting, logical/current-row composition, strict one-field output, shared request time, eight nesting levels, 32 lists, request-local caching, cumulative work/state/result limits, cancellation, optimize, flush, and reopen. `SQL-LOG-048` gives direct users the bounded two-scan retained-string foundation. Composition stays in Rust over public rows; no nested-query vtab, private-table access, extension primitive, or storage change is used. |
