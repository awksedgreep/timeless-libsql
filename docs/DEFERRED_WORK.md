...
current `0.7.x` source line. It keeps future sessions from depending on old
...
|---|---|---|
| Native release publication | Resolved: since `v0.4.2` the tag workflow publishes a GitHub Release after the four package jobs and the aggregate checksum gate pass; `v0.7.6` is the current published release with all four archives plus `SHA256SUMS` as permanent assets. | None — [ARTIFACTS.md](ARTIFACTS.md) stays the canonical per-tag publication record. |
...
| Broader competitive workloads | The [2026-09-13 comparison](2026-09-13_competitive_review.md) measures verified identical metric/log read workloads against pinned Prometheus, VictoriaMetrics and VictoriaLogs, with repeated native-container captures and explicit capability gaps. | Address count planning and log pipeline composition, then extend to sustained ingestion, concurrency, high cardinality, cold reads and trace/Jaeger workloads. The current captures do not establish general performance parity. |
