...
INSERT INTO dbhealth(dbhealth) VALUES ('sample');   -- snapshot counters now

SELECT ts, value FROM dbhealth
 WHERE name = 'cache_hit_ratio' AND ts > unixepoch() - 86400;
```

A companion vtab module for timeless-libsql: sample SQLite's own health
counters — cache efficiency, bloat, WAL growth, memory, scan behavior — into
a compressed time-series store *inside the same database file*. PMM for
SQLite, except there is no server, no agent, and no second system: the
database monitors itself, and the history travels with the file.
