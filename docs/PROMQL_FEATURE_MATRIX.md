...
| `PQL-F09` | `label_replace`; full-match dot-all regex, numbered/named captures, missing/empty sources, unmatched identity, destination deletion/overwrite, metric names, Prometheus 3 UTF-8 label names, range grids, nesting, errors, limits, cancellation, and reopen are pinned | shipped | yes | none | `API` | P0 |
