...
| `PQL-S05` | regex/negative `__name__` and multi-metric selection | in progress | yes | `CAT`, `RAW` | `API` | P0 | `session_three_promql_metric_name_matchers_prune_before_reads_and_reopen` pins anchored regex, negative and duplicate-name matcher behavior through the real extension. |
