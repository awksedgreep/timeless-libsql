...
The current Rust server supports exact-name instant vector selectors and
`avg_over_time(selector[window])`. Nothing else should be inferred from native
`metric=` query parameters or from extension kernels with similar names.
...
| `PQL-S19` | deterministic Prometheus error/result envelopes | in progress | yes | none | `API` | P0 | Pin required range parameters, parameter-scoped diagnostics, query type errors, strict unsupported behavior, and GET/POST envelope identity for the shipped grammar. |
