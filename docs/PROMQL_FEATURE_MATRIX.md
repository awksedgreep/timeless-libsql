...
| `MQL-09` | request-step-relative durations such as `[5i]`, `[5i:1i]`, and `offset 5i` ([SQL](QUERY_SQL_EQUIVALENTS.md#sql-mql-009-request-step-relative-durations)); decimal/compound/case behavior, inherited negative offsets, `int64` saturation, comments/strings, collision-free zero lowering, adaptive direct/subquery `0i`, limits, stable-route isolation, and reopen are implemented pending exact-build evidence | in progress | yes | `API` | P2 |
