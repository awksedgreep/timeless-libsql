...
| `PQL-H04` | `histogram_avg/count/sum/stddev/stdvar` on native samples | deferred | no | none | `DEFER` | DEFER | Stable float-input behavior is shipped: each function evaluates its child and ignores every float sample, including classic `_bucket`/`_sum`/`_count` series. Producing values remains deferred until the typed design in `PQL-S22`; see `QSF-259`. |
