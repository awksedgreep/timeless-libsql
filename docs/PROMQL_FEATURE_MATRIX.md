...
| `PQL-S09` | subqueries `[range:resolution]` ([SQL](QUERY_SQL_EQUIVALENTS.md#sql-prom-009-aligned-selector-subquery)) | in progress | yes | `RAW`, `GRID`, `SQL` | `API` | P0 | Prometheus 3.13.2 fixes open-left ranges, globally aligned explicit steps, the configurable 15-second default step, modifier ordering, and result typing; implementation must bound intermediate points and cancellation. |
