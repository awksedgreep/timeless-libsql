...
| `MQL-10` | query-context `start()`, `end()`, and `step()` ([SQL](QUERY_SQL_EQUIVALENTS.md#sql-mql-010-query-context-values)); request-owned range/instant values, subsecond/pre-epoch seconds, case-insensitive zero-argument grammar, expression composition, preserved selector `@ start()`/`@ end()`, stable-route isolation, explicit unsupported functions, limits, cancellation, and reopen are pinned | in progress | yes | `API` | P2 |
