...
Status: complete. Bounded equality prerequisites shipped; this remains a
...
|---|---|---|---|---|
| `TQP-01` | Exact typed span-attribute scalar equality | Bounded, opt-in exact-path candidate pruning plus exact per-span recheck | Parse TraceQL attribute syntax and compose the returned span set | shipped prerequisite |
| `TQP-02` | Exact typed resource-attribute scalar equality | Same primitive with an explicit resource scope | Resolve TraceQL resource namespace and compose span/trace results | shipped prerequisite |
| `TQP-03` | Exact typed instrumentation-scope scalar equality | Same primitive with an explicit scope scope | Resolve scope namespace and compose span/trace results | shipped prerequisite |
| `TQP-04` | Missing versus JSON null versus empty string | Equality keeps all three states distinct; ordinary public JSON1 remains the existence/type surface | Define language operators and null/missing propagation | shipped prerequisite |
...
The implementation ships because the final evidence accounts for ingest CPU,
logical/physical storage, WAL/checkpoint behavior, optimize and retention,
narrow/wide query tails and work, and RSS HWM. High-cardinality wide p95 is
38.6–40.6× faster; the documented low-cardinality and write-path taxes keep
configuration explicit. See the
