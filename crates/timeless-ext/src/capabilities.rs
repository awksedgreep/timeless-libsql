...
mod tests {
    ...
    fn document_is_stable_and_names_every_release_signal() {
        ...
        assert!(
            ...
            "the current extension must remain on the documented 0.7 compatibility line"
        );
        assert_eq!(
            value["minimum_server_version"], "0.7.0",
            "compatible 0.7 patch releases retain the 0.7.0 server floor"
        );
