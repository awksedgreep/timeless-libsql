...
impl Engine {
    ...
    pub fn ingest_prometheus(&self, body: &[u8], default_ts: i64) -> EngineResult<(usize, usize)> {
        ...
        let (count, errors) =
            crate::prom_text::parse_prom_body_visit(body, |name, labels, value, ts| {
                if failure.is_err() {
                    return;
                }
    ...
    fn resolve_escaped(&self, name: &[u8], labels: &[(&[u8], &[u8])]) -> EngineResult<i64> {
        let metric_storage = name
            .contains(&b'\\')
            .then(|| crate::prom_text::unescape_prom_label_value(name));
        ...
        let labels: HashMap<String, String> = labels
            ...
            .map(|&(raw_key, raw_value)| {
                let key_storage = raw_key
                    .contains(&b'\\')
                    .then(|| crate::prom_text::unescape_prom_label_value(raw_key));
                let value_storage = raw_value
                    .contains(&b'\\')
                    .then(|| crate::prom_text::unescape_prom_label_value(raw_value));
                (
