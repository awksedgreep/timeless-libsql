...
impl SpanBlockStore for PausingFlushStore {
...
fn stats_concurrent_with_flush_does_not_invert_index_and_buffer() {
    ...
    engine
        .push(span(1, 1, None, "buffered", "api", 1, 1, 1, &[]))
        .unwrap();
...
fn savepoint_rollback_is_repeatable_and_unwinds_nested_frames() {
    ...
    engine.txn_begin();
    engine
        .push(span(1, 1, None, "outer", "s", 0, 0, 1, &[]))
        .unwrap();
    engine.txn_savepoint(0);
    engine
        .push(span(1, 2, None, "inner-1", "s", 0, 0, 2, &[]))
        .unwrap();
    engine.txn_savepoint(1);
    engine
        .push(span(1, 3, None, "inner-2", "s", 0, 0, 3, &[]))
        .unwrap();

    engine.txn_rollback_to(0);
    assert_eq!(engine.buffered_count(), 1);
    engine
        .push(span(1, 4, None, "inner-again", "s", 0, 0, 4, &[]))
        .unwrap();
    engine.txn_rollback_to(0);
...
fn released_flush_frame_still_rolls_back_with_outer_transaction() {
    let engine =
        SpanBlockEngine::new(Box::new(MemSpanStore::new()), SpanEngineConfig::default()).unwrap();
    engine
        .push(span(1, 1, None, "pre", "s", 0, 0, 1, &[]))
        .unwrap();
    engine.txn_begin();
    engine
        .push(span(1, 2, None, "outer", "s", 0, 0, 2, &[]))
        .unwrap();
    engine.txn_savepoint(0);
    engine.flush().unwrap();
    engine.txn_release(0);
...
fn compression_totals_persist_across_reopen_and_credit_merges() {
    ...
    assert!(in3 > in1, "input side accumulates raw bytes only");
