...
//!    errors.
//! 5. Prometheus `\\n`, `\\\"`, and `\\\\` escapes in quoted metric names,
//!    label names, and label values are decoded before series identity is
//!    resolved. Prometheus 3 brace-first quoted UTF-8 names retain identity.
//!
//! If any of these change, the vtab's Prometheus path breaks — fail here
...
fn prometheus_ingest_semantics() {
    ...
    let utf8_body = r#"{"oracle.metric/温度","node.name"="東京"} 42 1800000000000
{"oracle.\"quoted\"\\温度","node.name"="大阪"} 43 1800000000000
"#
    .as_bytes();
    let (count, errors) = engine.ingest_prometheus(utf8_body, default_ts).unwrap();
    assert_eq!((count, errors), (2, 0));
    for (name, node, expected) in [
        ("oracle.metric/温度", "東京", 42.0),
        ("oracle.\"quoted\"\\温度", "大阪", 43.0),
    ] {
        let labels = HashMap::from([("node.name".to_owned(), node.to_owned())]);
        let sid = engine.resolve_cached(name, &labels).unwrap();
        assert_eq!(
            engine.query_range_by_id(sid, 0, i64::MAX - 1).unwrap(),
            vec![(1_800_000_000, expected)]
        );
    }
    ...
    assert_eq!(info.prometheus_ingest_batches, 4);
    assert_eq!(info.prometheus_ingest_points, 9);
