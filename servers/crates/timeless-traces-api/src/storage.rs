...
pub struct StorageStats {
    ...
    pub extension_query_decoded_columns: i64,
    pub extension_query_decoded_column_bytes: i64,
    pub extension_query_materialized_values: i64,
    pub extension_query_materialized_rich_values: i64,
...
fn optimize_all_backlog(conn: &Connection) -> Result<(), String> {
    for step in 0..MAX_BACKUP_OPTIMIZE_STEPS {
        ...
        if !optimize_made_progress(before, after) {
            return Err(format!(
                "traces optimize backlog made no progress at step {step}: {after:?}"
            ));
        }
    }
    Err(format!(
        "traces optimize backlog exceeded {MAX_BACKUP_OPTIMIZE_STEPS} steps"
    ))
}
...
fn storage_stats(conn: &Connection) -> Result<StorageStats, String> {
    ...
    Ok(StorageStats {
        ...
        extension_query_decoded_columns: integer("query_decoded_columns"),
        extension_query_decoded_column_bytes: integer("query_decoded_column_bytes"),
        extension_query_materialized_values: integer("query_materialized_values"),
        extension_query_materialized_rich_values: integer("query_materialized_rich_values"),
