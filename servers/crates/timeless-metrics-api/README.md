...
in Rust. `rate`, `irate`, `increase`, `delta`, `idelta`, and `deriv` use the
same bounded packed raw path so the Rust evaluator can apply their distinct counter/gauge
