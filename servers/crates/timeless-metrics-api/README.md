...
in Rust. `rate` uses the same bounded packed raw path so the Rust evaluator can
apply complete counter and extrapolation semantics. A root range selector reads public raw frames,
