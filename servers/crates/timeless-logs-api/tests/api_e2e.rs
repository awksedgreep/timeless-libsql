...
#[tokio::test(flavor = "multi_thread")]
#[ignore = "requires TIMELESS_EXT_TEST_PATH pointing at libtimeless_ext"]
async fn session_nineteen_stream_selectors_fail_explicitly_without_storage() {
    let extension = std::env::var("TIMELESS_EXT_TEST_PATH")
        .expect("TIMELESS_EXT_TEST_PATH must point at libtimeless_ext");
    let temp = tempfile::tempdir().unwrap();
    let database = temp.path().join("deferred-stream-selector-logsql.db");
    let storage = Storage::start_with_timestamp_unit(
        database.clone(),
        extension.clone().into(),
        1,
    ...
    assert_eq!(
        app.clone()
            .oneshot(ingest_request(
                r#"{"_time":1800000000000001,"_msg":"literal { brace }","level":"notice","service":"api","nested":{"value":7},"array":[1,true,null,"x"]}"#
                    .to_owned(),
            ))
            .await
            .unwrap()
    ...
    async fn assert_rejected(app: &axum::Router, request: Request<Body>, message: &str) {
    ...
    let quoted = app
        .clone()
        .oneshot(logsql_request(r#""literal { brace }""#))
        .await
        .unwrap();
    assert_eq!(quoted.status(), StatusCode::OK);
    let rows = ndjson_values(&to_bytes(quoted.into_body(), usize::MAX).await.unwrap());
    assert_eq!(rows.len(), 1);
    assert_eq!(rows[0]["nested"], serde_json::json!({"value": 7}));
    assert_eq!(rows[0]["array"], serde_json::json!([1, true, null, "x"]));

    let before = storage.stats().await.unwrap();
    assert_rejected(
        &app,
        logsql_request(r#"{case="one"}"#),
        "LogsQL stream selector at line 1, column 1 is deferred: Timeless does not store a VictoriaLogs-compatible stream identity",
    )
    .await;
    assert_rejected(
        &app,
        logsql_request(r#"_stream:{case=~"one|two"}"#),
        "LogsQL stream selector at line 1, column 9 is deferred: Timeless does not store a VictoriaLogs-compatible stream identity",
    )
    .await;
    assert_rejected(
        &app,
        logsql_request("*\n| filter {service=\"api\"}"),
        "LogsQL stream selector at line 2, column 10 is deferred: Timeless does not store a VictoriaLogs-compatible stream identity",
    )
    .await;
    let after = storage.stats().await.unwrap();
    ...
    let quoted = reopened_app
        .oneshot(logsql_request(r#""literal { brace }""#))
        .await
        .unwrap();
    assert_eq!(quoted.status(), StatusCode::OK);
    assert_eq!(
        ndjson_values(&to_bytes(quoted.into_body(), usize::MAX).await.unwrap()).len(),
        1
    );
    reopened.shutdown().await.unwrap();
}
...
#[tokio::test(flavor = "multi_thread")]
#[ignore = "requires TIMELESS_EXT_TEST_PATH pointing at libtimeless_ext"]
async fn session_nineteen_stream_context_fails_explicitly_without_storage() {
    let extension = std::env::var("TIMELESS_EXT_TEST_PATH")
        .expect("TIMELESS_EXT_TEST_PATH must point at libtimeless_ext");
    let temp = tempfile::tempdir().unwrap();
    let database = temp.path().join("deferred-stream-context-logsql.db");
    let storage = Storage::start_with_timestamp_unit(
        database.clone(),
        extension.clone().into(),
        1,
    ...
    assert_eq!(
        app.clone()
            .oneshot(ingest_request(
                r#"{"_time":1800000000000001,"_msg":"literal stream_context value","level":"notice","service":"api","case":"ordinary","nested":{"stream_context":"ordinary","value":7},"array":[1,true,null,"x"]}"#
                    .to_owned(),
            ))
            .await
            .unwrap()
    ...
    async fn assert_rejected(app: &axum::Router, query: &str, line: usize, column: usize) {
        let response = app.clone().oneshot(logsql_request(query)).await.unwrap();
        assert_eq!(
            response.status(),
            StatusCode::UNPROCESSABLE_ENTITY,
    ...
    let ordinary = app
        .clone()
        .oneshot(logsql_request(r#""literal stream_context value""#))
        .await
        .unwrap();
    assert_eq!(ordinary.status(), StatusCode::OK);
    let rows = ndjson_values(&to_bytes(ordinary.into_body(), usize::MAX).await.unwrap());
    ...
    assert_eq!(
        rows[0]["nested"],
        serde_json::json!({"stream_context": "ordinary", "value": 7})
    );
    assert_eq!(rows[0]["array"], serde_json::json!([1, true, null, "x"]));

    let before = storage.stats().await.unwrap();
    assert_rejected(&app, "* | stream_context before 1", 1, 5).await;
    assert_rejected(&app, "*\n| STREAM_CONTEXT after 1 time_window 2h30m", 2, 3).await;
    assert_rejected(&app, "* | limit 1 | stream_context after 1", 1, 15).await;
    assert_rejected(
        &app,
        "case:in(* | stream_context before 1 | fields case)",
        1,
        13,
    )
    .await;
    ...
    let reopened_app = router(reopened.clone());
    let before = reopened.stats().await.unwrap();
    assert_rejected(&reopened_app, "* | stream_context before 0 after 0", 1, 5).await;
    let after = reopened.stats().await.unwrap();
    assert_eq!(after.api_query_count, before.api_query_count);
    assert_eq!(after.query_count, before.query_count);
    assert_eq!(after.native_count_count, before.native_count_count);
    ...
    let ordinary = reopened_app
        .oneshot(logsql_request(r#"nested.stream_context:"ordinary""#))
        .await
        .unwrap();
    assert_eq!(ordinary.status(), StatusCode::OK);
    assert_eq!(
...
#[tokio::test(flavor = "multi_thread")]
#[ignore = "requires TIMELESS_EXT_TEST_PATH pointing at libtimeless_ext"]
async fn session_nineteen_query_parallelism_options_fail_explicitly_without_storage() {
    let extension = std::env::var("TIMELESS_EXT_TEST_PATH")
        .expect("TIMELESS_EXT_TEST_PATH must point at libtimeless_ext");
    let temp = tempfile::tempdir().unwrap();
    let database = temp.path().join("deferred-query-parallelism-logsql.db");
    let storage = Storage::start_with_timestamp_unit(
        database.clone(),
        extension.clone().into(),
        2,
    ...
    assert_eq!(
        app.clone()
            .oneshot(ingest_request(
                r#"{"_time":1800000000000001,"_msg":"options(concurrency=1)","level":"notice","service":"api","case":"ordinary","nested":{"concurrency":1,"parallel_readers":2},"array":[1,true,null,"x"]}"#
                    .to_owned(),
            ))
            .await
            .unwrap()
    ...
    let ordinary = app
        .clone()
        .oneshot(logsql_request(r#""options(concurrency=1)""#))
        .await
        .unwrap();
    assert_eq!(ordinary.status(), StatusCode::OK);
    let rows = ndjson_values(&to_bytes(ordinary.into_body(), usize::MAX).await.unwrap());
    ...
    assert_eq!(
        rows[0]["nested"],
        serde_json::json!({"concurrency": 1, "parallel_readers": 2})
    );
    assert_eq!(rows[0]["array"], serde_json::json!([1, true, null, "x"]));

    let before = storage.stats().await.unwrap();
    assert_rejected(&app, "options(concurrency=1) *", "concurrency", 1, 9).await;
    assert_rejected(
        &app,
    ...
    assert_rejected(
        &app,
        "case:in(options(concurrency=1) case:=ordinary | fields case)",
        "concurrency",
        1,
        17,
    )
    ...
    let ordinary = reopened_app
        .oneshot(logsql_request("nested.concurrency:=1"))
        .await
        .unwrap();
    assert_eq!(ordinary.status(), StatusCode::OK);
    assert_eq!(
