...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum RunningStatsMode {
    Running,
    Total,
}
...
fn parse_running_stats_pipe(
    ...
    let command = segment
        .get(..operation.len())
        .ok_or_else(|| LogsqlError::malformed(format!("LogsQL {operation} pipe is empty")))?;
    if !command.eq_ignore_ascii_case(operation) {
        return Err(LogsqlError::malformed(format!(
    ...
    {
        return Err(LogsqlError::malformed(format!(
            "LogsQL {operation} requires whitespace before its arguments"
        )));
    }
    ...
    if starts_format_keyword(rest, "by") {
        ...
        let (fields, tail) = parse_running_stats_group(rest, operation)?;
        group_by = fields;
        rest = tail;
    ...
    if rest.is_empty() {
        return Err(LogsqlError::malformed(format!(
            "LogsQL {operation} requires at least one function"
        )));
    }

    let expressions = split_top_level(rest, ',')?
        .into_iter()
        .map(|expression| parse_running_stats_expression(expression.trim(), operation))
        .collect::<Result<Vec<_>, _>>()?;
    let mut aliases = BTreeSet::new();
    for expression in &expressions {
        if !aliases.insert(expression.alias_name.clone()) {
            return Err(LogsqlError::malformed(format!(
                "duplicate LogsQL {operation} result name {:?}",
...
fn parse_running_stats_group<'a>(
    rest: &'a str,
    operation: &str,
) -> Result<(Vec<PipelineField>, &'a str), LogsqlError> {
    if !rest.starts_with('(') {
        return Err(LogsqlError::malformed(format!(
            "LogsQL {operation} by requires parenthesized fields"
        )));
    }
    let close = matching_parenthesis(rest, 0).map_err(|_| {
        LogsqlError::malformed(format!("unterminated LogsQL {operation} group fields"))
    })?;
    let source = rest[1..close].trim();
    if source.is_empty() {
        return Err(LogsqlError::malformed(format!(
            "LogsQL {operation} group requires at least one field"
        )));
    }
    let fields = split_top_level(source, ',')?
        .into_iter()
        .map(|field| parse_running_stats_exact_field(field.trim(), "group", operation))
        .collect::<Result<Vec<_>, _>>()?;
    Ok((fields, rest[close + 1..].trim_start()))
}

fn parse_running_stats_expression(
    expression: &str,
    operation: &str,
) -> Result<RunningStatsExpression, LogsqlError> {
    let open = expression.find('(').ok_or_else(|| {
        LogsqlError::malformed(format!(
            "LogsQL {operation} function requires parentheses: {expression:?}"
        ))
    })?;
    let close = matching_parenthesis(expression, open).map_err(|_| {
        LogsqlError::malformed(format!(
            "unterminated LogsQL {operation} function {expression:?}"
        ))
    })?;
    let function_source = expression[..open].trim();
    let function = function_source.to_ascii_lowercase();
    ...
    let kind = match function.as_str() {
        ...
        "first" | "last" => {
            if fields.len() != 1 {
                return Err(LogsqlError::malformed(format!(
                    "LogsQL {operation} {function} requires exactly one field"
                )));
            }
            if !matches!(fields.first(), Some(PipelineField::Exact { .. })) {
                return Err(LogsqlError::malformed(format!(
                    "LogsQL {operation} {function} requires an exact field"
                )));
            }
            if tail
                .first()
                .is_some_and(|token| token.eq_ignore_ascii_case("offset"))
            {
                if tail.len() < 2 {
                    return Err(LogsqlError::malformed(format!(
                        "LogsQL {operation} {function} offset requires a value"
                    )));
                }
                offset = parse_pipeline_usize(&format!("{operation} {function} offset"), &tail[1])?;
                tail.drain(..2);
            }
            if function == "first" {
        ...
        _ => {
            return Err(LogsqlError::malformed(format!(
                "unsupported LogsQL {operation} function {function_source:?}"
            )))
        }
    };
    ...
    let explicit_alias = match tail.as_slice() {
        ...
        [keyword] if keyword.eq_ignore_ascii_case("as") => {
            return Err(LogsqlError::malformed(format!(
                "LogsQL {operation} {function} alias requires a value"
            )))
        }
        ...
        _ => {
            return Err(LogsqlError::malformed(format!(
                "unexpected LogsQL {operation} {function} token {:?}",
...
fn parse_running_stats_exact_field(
    source: &str,
    role: &str,
    operation: &str,
    ...
    match parse_pipeline_field(source, false)? {
        ...
        PipelineField::Prefix { .. } | PipelineField::All => Err(LogsqlError::malformed(format!(
            "LogsQL {operation} {role} requires an exact field"
        ))),
    }
}
...
fn parse_join_pipe(segment: &str, context: &mut ParseContext) -> Result<PipelineOp, LogsqlError> {
    const OPERATION: &str = "join";
    let command = segment
        .get(..OPERATION.len())
        .ok_or_else(|| LogsqlError::malformed("LogsQL join pipe is empty"))?;
    if !command.eq_ignore_ascii_case(OPERATION) {
        return Err(LogsqlError::malformed(format!(
            "expected LogsQL join pipe, not {command:?}"
        )));
    }
    let command_tail = &segment[OPERATION.len()..];
    if command_tail
    ...
    {
        return Err(LogsqlError::malformed(
            "LogsQL join requires whitespace before its fields",
        ));
    }
    let mut rest = command_tail.trim_start();
    if starts_format_keyword(rest, "by") {
        rest = rest["by".len()..].trim_start();
    } else if starts_format_keyword(rest, "on") {
        rest = rest["on".len()..].trim_start();
    }
    if !rest.starts_with('(') {
        return Err(LogsqlError::malformed(
            "LogsQL join requires parenthesized exact fields",
        ));
    }
    let fields_close = matching_parenthesis(rest, 0)
        .map_err(|_| LogsqlError::malformed("unterminated LogsQL join fields"))?;
    let fields_source = rest[1..fields_close].trim();
    if fields_source.is_empty() {
        return Err(LogsqlError::malformed(
            "LogsQL join requires at least one exact field",
        ));
    }
    let fields_source = fields_source.strip_suffix(',').unwrap_or(fields_source);
    let fields = split_top_level(fields_source, ',')?
        .into_iter()
        .map(|field| parse_first_exact_field(field.trim(), "by", OPERATION))
        .collect::<Result<Vec<_>, _>>()?;
    if fields.is_empty() {
        return Err(LogsqlError::malformed(
            "LogsQL join requires at least one exact field",
    ...
    rest = rest[fields_close + 1..].trim_start();
    if rest.is_empty() {
        return Err(LogsqlError::malformed(
    ...
    let mut tail = tail;
    while !tail.is_empty() {
        if starts_format_keyword(tail, "inner") {
            if inner {
                return Err(LogsqlError::malformed(
                    "LogsQL join inner modifier may appear only once",
                ));
            }
            inner = true;
            tail = tail["inner".len()..].trim_start();
            continue;
        }
        if starts_format_keyword(tail, "prefix") {
            if prefix.is_some() {
                return Err(LogsqlError::malformed(
                    "LogsQL join prefix modifier may appear only once",
                ));
            }
            tail = tail["prefix".len()..].trim_start();
            let (value, remaining) = take_join_compound_token(tail, "prefix")?;
            prefix = Some(value);
            tail = remaining.trim_start();
            continue;
        }
        return Err(LogsqlError::malformed(format!(
            "unexpected LogsQL join token {tail:?}"
        )));
    }

    Ok(PipelineOp::Join(JoinSpec {
        fields,
        source,
        inner,
...
fn take_join_compound_token<'a>(
    value: &'a str,
    role: &str,
) -> Result<(String, &'a str), LogsqlError> {
    if value.is_empty() {
        return Err(LogsqlError::malformed(format!(
            "LogsQL join {role} requires a value"
        )));
    }
    if let Some((decoded, consumed)) = parse_quoted_prefix(value)? {
        return Ok((decoded, &value[consumed..]));
    }
    ...
    {
        return Err(LogsqlError::malformed(format!(
            "invalid LogsQL join {role} {token:?}"
        )));
    }
...
fn take_inline_atom(
    ...
    if let Some((_decoded, consumed)) = parse_quoted_prefix(rest)? {
        let token = rest[..consumed].to_owned();
        *cursor += consumed;
        return Ok(token);
    }
    let mut end = *cursor;
...
fn is_total_stats_pipe(segment: &str) -> bool {
    let operation = "total_stats";
    segment
        .get(..operation.len())
        .is_some_and(|command| command.eq_ignore_ascii_case(operation))
}
...
fn is_pack_logfmt_pipe(segment: &str) -> bool {
    let operation = "pack_logfmt";
    segment
        .get(..operation.len())
        .is_some_and(|command| command.eq_ignore_ascii_case(operation))
}
...
fn is_unpack_logfmt_pipe(segment: &str) -> bool {
    let operation = "unpack_logfmt";
    segment
        .get(..operation.len())
        .is_some_and(|command| command.eq_ignore_ascii_case(operation))
}
...
fn parse_pack_json_pipe(segment: &str) -> Result<PipelineOp, LogsqlError> {
    ...
    {
        cursor += 1;
        fields = parse_pack_fields(&tokens, &mut cursor, operation)?;
        if fields
            .iter()
...
fn parse_pack_fields(
    ...
    if tokens.get(*cursor).is_none_or(|token| token != "(") {
        return Err(LogsqlError::malformed(format!(
            "LogsQL {operation} fields requires parenthesized fields"
        )));
    }
    *cursor += 1;
    let mut fields = Vec::new();
    loop {
        match tokens.get(*cursor).map(String::as_str) {
            ...
            Some(",") | None => {
                return Err(LogsqlError::malformed(format!(
                    "LogsQL {operation} fields requires a field after each comma"
                )));
            }
            Some(token) => {
                ...
                *cursor += 1;
                match tokens.get(*cursor).map(String::as_str) {
                    ...
                    Some(token) => {
                        return Err(LogsqlError::malformed(format!(
                            "unexpected LogsQL {operation} fields token {token:?}; expected ',' or ')'"
                        )));
                    }
                    None => {
                        return Err(LogsqlError::malformed(format!(
                            "unterminated LogsQL {operation} fields"
                        )));
                    }
                }
            }
...
fn parse_pack_logfmt_pipe(segment: &str) -> Result<PipelineOp, LogsqlError> {
    let operation = "pack_logfmt";
    let tokens = lex_first_pipe(segment, operation)?;
    let Some(command) = tokens.first() else {
        return Err(LogsqlError::malformed("LogsQL pack_logfmt pipe is empty"));
    };
    if !command.eq_ignore_ascii_case(operation) {
        return Err(LogsqlError::malformed(format!(
            "expected LogsQL pack_logfmt pipe, not {command:?}"
        )));
    }
    ...
    if let Some(token) = tokens.get(cursor) {
        if matches!(token.as_str(), "(" | ")" | ",") {
            return Err(LogsqlError::malformed(
                "LogsQL pack_logfmt result must be an exact field",
            ));
        }
        destination = match parse_delete_field(token)? {
            field @ PipelineField::Exact { .. } => field,
    ...
    if let Some(token) = tokens.get(cursor) {
        return Err(LogsqlError::malformed(format!(
            "unexpected LogsQL pack_logfmt token {token:?}"
        )));
    }

    Ok(PipelineOp::PackLogfmt(PackLogfmtSpec {
        fields,
        destination,
    }))
}
...
fn parse_unpack_json_pipe(
    ...
    let parsed = parse_unpack_pipe(segment, context, "unpack_json", true)?;
    Ok(PipelineOp::UnpackJson(UnpackJsonSpec {
        source: parsed.source,
        fields: parsed.fields,
        preserve_keys: parsed.preserve_keys,
        result_prefix: parsed.result_prefix,
...
fn parse_unpack_logfmt_pipe(
    segment: &str,
    context: &mut ParseContext,
) -> Result<PipelineOp, LogsqlError> {
    let parsed = parse_unpack_pipe(segment, context, "unpack_logfmt", false)?;
    debug_assert!(parsed.preserve_keys.is_empty());
    Ok(PipelineOp::UnpackLogfmt(UnpackLogfmtSpec {
        source: parsed.source,
        fields: parsed.fields,
        result_prefix: parsed.result_prefix,
        keep_original_fields: parsed.keep_original_fields,
...
struct ParsedUnpackSpec {
    ...
    fields: Vec<PipelineField>,
    preserve_keys: Vec<PipelineField>,
    result_prefix: String,
    keep_original_fields: bool,
    skip_empty_results: bool,
    condition: Option<LogPredicate>,
}

fn parse_unpack_pipe(
    segment: &str,
    context: &mut ParseContext,
    operation: &str,
    allow_preserve_keys: bool,
) -> Result<ParsedUnpackSpec, LogsqlError> {
    let command = segment
        .get(..operation.len())
        .ok_or_else(|| LogsqlError::malformed(format!("LogsQL {operation} pipe is empty")))?;
    if !command.eq_ignore_ascii_case(operation) {
    ...
    {
        return Err(LogsqlError::malformed(format!(
            "unexpected text attached to LogsQL {operation} pipe"
        )));
    }
    let mut rest = segment[operation.len()..].trim_start();
    let condition = if starts_format_keyword(rest, "if") {
    ...
    let source_is_implicit = tokens.get(cursor).is_none_or(|token| {
        ...
        .iter()
        .any(|keyword| token.eq_ignore_ascii_case(keyword))
            || (allow_preserve_keys && token.eq_ignore_ascii_case("preserve_keys"))
    });
    if !source_is_implicit {
        if tokens
            .get(cursor)
        ...
        let token = tokens.get(cursor).ok_or_else(|| {
            LogsqlError::malformed(format!(
                "LogsQL {operation} from requires an exact source field"
            ))
        })?;
        source = match parse_delete_field(token)? {
            field @ PipelineField::Exact { .. } => field,
    ...
    {
        cursor += 1;
        fields = parse_unpack_field_list(&tokens, &mut cursor, operation, "fields", true)?;
        if fields
            .iter()
            .any(|field| matches!(field, PipelineField::All))
        {
            ...
            .is_some_and(|token| token.eq_ignore_ascii_case("preserve_keys"))
    {
        cursor += 1;
        preserve_keys =
            parse_unpack_field_list(&tokens, &mut cursor, operation, "preserve_keys", false)?;
    }
        ...
        .is_some_and(|token| token.eq_ignore_ascii_case("result_prefix"))
    {
        ...
        let token = tokens.get(cursor).ok_or_else(|| {
            LogsqlError::malformed(format!(
                "LogsQL {operation} result_prefix requires a prefix"
            ))
        })?;
        if matches!(token.as_str(), "(" | ")" | ",") {
            return Err(LogsqlError::malformed(format!(
        ...
        match tokens.get(*cursor).map(String::as_str) {
            ...
            Some(token) => {
                ...
                if !allow_wildcards && !matches!(field, PipelineField::Exact { .. }) {
                    return Err(LogsqlError::malformed(format!(
                        "LogsQL {operation} preserve_keys requires exact fields"
                    )));
                }
                fields.push(field);
                *cursor += 1;
                match tokens.get(*cursor).map(String::as_str) {
                    ...
                    Some(token) => {
                        return Err(LogsqlError::malformed(format!(
                            "LogsQL {operation} {clause} requires commas, not {token:?}"
                        )));
                    }
                    None => {
                        return Err(LogsqlError::malformed(format!(
                            "unterminated LogsQL {operation} {clause} list"
                        )));
                    }
                }
            }
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct PackLogfmtSpec {
    pub fields: Vec<PipelineField>,
    pub destination: PipelineField,
}
...
#[derive(Clone, Debug)]
pub(crate) struct UnpackLogfmtSpec {
    pub source: PipelineField,
    pub fields: Vec<PipelineField>,
    pub result_prefix: String,
    pub keep_original_fields: bool,
    ...
    for segment in segments {
        ...
        match words.as_slice() {
            ...
            _ if is_running_stats_pipe(segment) => {
                pipeline.push(parse_running_stats_pipe(
                    segment,
                    RunningStatsMode::Running,
                )?);
                has_session_thirteen_pipeline = true;
            }
            ...
            _ if is_pack_logfmt_pipe(segment) => {
                pipeline.push(parse_pack_logfmt_pipe(segment)?);
                has_session_thirteen_pipeline = true;
            }
...
mod tests {
    ...
    #[test]
    fn session_eighteen_join_grammar_is_complete_and_strict() {
        for query in [
            "* | join by (case) (kind:=admin | fields case, value)",
            "* | join on (case, kind) (kind:=admin | fields case, kind, value) inner prefix right.",
            r#"* | join (case) rows({"case":"left","value":"x"})"#,
            r#"* | JoIn By ("left field", nested.key) rows({"left field":"x","nested.key":"y","value":"z"}) prefix "p." inner"#,
            "* | join by (case) rows() inner",
            "* | join by (case) rows({})",
            r#"* | join by (case) rows({ "case" : "a" , } { value : -1.24/sd-f },)"#,
            "* | join by (case) (* | join by (nested) (kind:=admin))",
        ] {
            let plan = parse_at(query, TimestampUnit::Microseconds, 0)
                .unwrap_or_else(|error| panic!("{query:?}: {error:?}"));
            assert_eq!(plan.output, LogsqlOutput::Pipeline, "{query:?}");
        ...
        for malformed in [
            ...
            "* | join by (case) rows({,})",
            r#"* | join by (case) rows({"case":"a",,})"#,
            r#"* | join by (case) rows({"case":[]})"#,
            r#"* | join by (case) rows({"case":{}})"#,
            "* | join by (case) rows({case: two words})",
            "* | join by (case) (*) prefix",
    ...
    fn session_eighteen_running_stats_grammar_is_complete_and_strict() {
        ...
        assert_eq!(spec.mode, RunningStatsMode::Running);
    ...
    #[test]
    fn session_eighteen_total_stats_grammar_is_complete_and_strict() {
        for query in [
            "* | total_stats count() as total",
            "* | TOTAL_STATS by (service, level) sum(value) total, count(p*) as seen",
            "* | total_stats (service) first(message) offset 2 as initial, last(message) previous",
            r#"* | total_stats by ("group name") min(value) as "minimum value", max(*)"#,
            "* | total_stats count(), sum(value) as total | filter total:>0",
        ] {
            let plan = parse_at(query, TimestampUnit::Microseconds, 0)
                .unwrap_or_else(|error| panic!("{query:?}: {error:?}"));
            assert_eq!(plan.output, LogsqlOutput::Pipeline, "{query:?}");
        ...
        let plan = parse_at(
            "* | total_stats by (service) count(), first(value) offset 2 as initial, last(value) previous",
            TimestampUnit::Microseconds,
            0,
        )
        .unwrap();
        let [PipelineOp::RunningStats(spec)] = plan.pipeline.as_slice() else {
            panic!("unexpected total_stats plan: {plan:?}");
        };
        assert_eq!(spec.mode, RunningStatsMode::Total);
        assert_eq!(spec.group_by.len(), 1);
        assert_eq!(spec.expressions.len(), 3);
        assert_eq!(spec.expressions[0].alias_name, "count(*)");
        assert_eq!(
    ...
    #[test]
    fn session_eighteen_pack_logfmt_grammar_is_complete_and_strict() {
        for query in [
            "* | pack_logfmt",
            "* | pack_logfmt as packed",
            "* | pack_logfmt packed",
            "* | pack_logfmt as",
            "* | pack_logfmt fields (case, nested.value, missing) as packed",
            r#"* | PaCk_LoGfMt FiElDs ("nested."*, flag, *,) As "packed field""#,
        ] {
            parse_at(query, TimestampUnit::Microseconds, 0)
                .unwrap_or_else(|error| panic!("{query}: {error}"));
        }
    ...
    #[test]
    fn session_eighteen_unpack_logfmt_grammar_is_complete_and_strict() {
        for query in [
            "* | unpack_logfmt",
            "* | unpack_logfmt source",
            "* | unpack_logfmt from source fields (case, nested.*, missing)",
            r#"* | UnPaCk_LoGfMt if (kind:=admin) FrOm "source field" FiElDs (nested.*, value) ReSuLt_PrEfIx "decoded." KeEp_OrIgInAl_FiElDs"#,
            "* | unpack_logfmt fields () skip_empty_results",
            "* | unpack_logfmt fields (case,)",
        ] {
            parse_at(query, TimestampUnit::Microseconds, 0)
                .unwrap_or_else(|error| panic!("{query}: {error}"));
        }
        ...
        for query in [
            "* | unpack_logfmt if",
            "* | unpack_logfmt if (case:=x) from",
            "* | unpack_logfmt from *",
            "* | unpack_logfmt from source trailing",
            "* | unpack_logfmt fields",
            "* | unpack_logfmt fields source",
