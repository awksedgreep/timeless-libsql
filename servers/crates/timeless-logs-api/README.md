...
A leading `options(time_offset=<duration>)` shifts logical storage bounds
backward and returned `_time` values forward with exact nanosecond response
precision. Nested queries inherit the value unless they explicitly replace
it; signed compound durations, duplicate-last assignments, day/week
composition, strict errors, limits, cancellation, optimize, and reopen are
...
p95 is 5.7%/7.7% higher, while both pairs read exactly the same one/four
blocks, decode 1,024/8,192 entries, cross 235,778/1,914,055 payload bytes,
and materialize 128/8,192 public rows. The 3,663-byte typed-object response
...
Exact-build `unpack_logfmt` evidence measures 3.619/42.437 ms narrow/wide
p95 versus 3.573/41.659 ms for identical-output pack-plus-copy controls. The
accepted +1.3%/+1.9% p95 and +1.2%/+5.9% internal API mean perform
byte-identical public storage work and emit the same 2,112-byte responses.
Quoted/unquoted parsing, escape decoding, dynamic selection, and retained
...
Exact-build `unpack_syslog` evidence measures 3.581/38.081 ms narrow/wide
p95 versus 3.439/38.026 ms for identical-output format-plus-copy controls.
The accepted +4.1%/+0.1% p95 and +9.5%/-0.8% internal API mean perform
byte-identical public storage work and emit the same 1,984-byte responses.
The benchmark limits to 64 rows before expansion after fully materializing the
one-/four-block public scan. Expanding all 8,192 formatted rows correctly
exceeds the default work budget; callers must narrow/limit first or configure
...
Exact-build `unpack_words` p50/p95/p99 is 3.225/3.740/4.877 ms narrow and
...
includes exact tokenization, deduplication, compact JSON-array encoding, and
640 additional response bytes after identical public block, decode, payload,
...
38.555/40.200/40.598 ms for equal-output format controls. Candidate/control
pairs perform identical public storage work and return identical 64-row,
1,536-byte responses; `QSF-229` records the +13.3%/-12.4% p95 and
-1.1%/-8.4% request-attributed API mean without inventing storage pushdown.
...
35.071/39.121/40.075 ms wide for 128 rows and 2,112 bytes, versus
...
request-attributed API mean after identical public storage work. The doubled
...
measures the 64-row query-backed join at 6.568/6.875/7.299 ms narrow and
66.254/73.000/73.984 ms wide p50/p95/p99. Equal-output query-backed
`in(...)` controls measure 5.934/6.225/6.666 and
65.809/72.087/72.452 ms. Join p95 is 10.4%/1.3% higher and
request-attributed API work is 14.7%/0.5% higher. Both sides perform exactly
two public scans and have byte-identical physical block/decode/payload/row
work; the join additionally reserves exactly 192 bounded work items for its
64 retained two-field right rows. The accepted difference is request-local
...
measures query-backed union at 6.794/74.143 ms narrow/wide p95 versus
6.363/73.714 ms equal-output two-scan controls, or +6.8%/+0.6%.
Request-attributed API means are 5.370/67.574 ms versus 4.931/67.884 ms, or
+8.9%/-0.5%. Every request returns identical 128-row, 2,560-byte output and
performs two public scans with byte-identical block, decode, payload, match,
and public-row work. Union reserves exactly 128 additional work items per
...
Matrix row `LQL-S14` is the function-catalog view of this same pipe: upstream
defines running `count`, `last`, `min`, `max`, and `sum` only inside
`running_stats`. It does not define standalone `running_*` function names.
The P45 implementation, `SQL-LOG-059`, oracle corpus, real-extension test,
and exact-build evidence therefore cover S14 without a duplicate API.

Exact release build `3f4ef107973f73361cfd90eff6e31ea53bd58f0c`
measures grouped running count at 3.437/48.000 ms narrow/wide p95 versus
3.771/38.690 ms same-scan time-sort/constant controls, or -8.9%/+24.1%.
Request-attributed API means are 2.467/44.051 ms versus 2.554/33.894 ms, or
-3.4%/+30.0%. Every request performs one public scan with byte-identical
block, decode, payload, match, requested-work, and public-row counters. The
candidate/control return the same 64 rows but 951/1,024 bytes because their
projected fields differ. The bounded wide partition/sort/state cost remains
...
Matrix row `LQL-S15` is the function-catalog view of this same pipe: upstream
defines total `count`, `first`, `last`, `min`, `max`, and `sum` only inside
`total_stats`. It does not define standalone `total_*` function names. The
P46 implementation, `SQL-LOG-060`, oracle corpus, real-extension test, and
exact-build evidence therefore cover S15 without a duplicate API.
...
versus quoted `math` constants. The bounded wide complete-group prepass and
...
10.1%/15.2% higher; request-attributed API means are 2.722/35.182 ms versus
2.529/32.495 ms, or 7.6%/8.3% higher. Both pairs have byte-identical public
block, decode, payload, match, return, and requested-work counters. The cost
...
measures 64 rows/886 bytes at 0.691/0.836/0.900 ms for an indexed-host source
form and 0.640/0.780/0.839 ms for a full-fixture source form p50/p95/p99.
Each shape executes 50 API requests with zero public query, native-count,
candidate-block, decode, payload, match, and returned-row work. The 6.6% lower
wide-form p95 is ordinary loopback variation between the same scan-free plan,
not source-dependent storage behavior. The complete workload leaves all 8,192
...
measures 64-row/1,856-byte results at 3.040/4.564/5.399 ms narrow and
